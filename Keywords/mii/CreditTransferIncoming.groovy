...
public class CreditTransferIncoming {

	@Keyword
	def comparereasonCode(def bodyResponse, String message){
		println(bodyResponse)
		def parser = new JsonSlurper().parseText(bodyResponse)
		println('parser' +parser)

		def result = parser['receivingCreditTransferResponse']['reasonCode']

		println("reasonCode" + result)
	...
	def compareResponseMessage(def bodyResponse, String message){
		...
		def result = parser['creditTransferResponse']['responseMessage']
